...
cmd /c "git checkout ${githubBranch} 2>&1"
...
$Action = New-ScheduledTaskAction -Execute "PowerShell.exe" -Argument "-ExecutionPolicy Bypass -File ${scriptDir}\MonitorWorkbookLogonScript.ps1 -spnClientId ${spnClientId} -spnClientSecret ${spnClientSecret} -spnTenantId ${spnTenantId} -resourceGroup ${resourceGroup} -subscriptionId ${subscriptionId} -workspaceName ${workspaceName}"
